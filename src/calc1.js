...
function calcTi(lamda, mu, K) {
  ...
  while (true) {
    roundTi -= arrivalTime;
    let check =
      Math.trunc(lamda * roundTi) - Math.trunc(mu * roundTi - mu / lamda);
    if (check != K) {
      break;
    }
  }
...
function getLamdaeval() {
  let getLamda;
  try {
    getLamda = eval(document.getElementById("lamda").value) + 0.00001;
  } catch (err) {
    document.getElementById("hint").classList.remove("d-none");
...
function getMueval() {
  let getMu;
  try {
    getMu = eval(document.getElementById("mu").value) + 0.00001;
  } catch (err) {
    document.getElementById("hint").classList.remove("d-none");
    document.getElementById("hint").innerHTML = "invalid input..!";
    document.getElementById("nt").innerHTML = "n(t) = ----";
