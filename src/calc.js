  ...
  if (getLamda > getMu) {
    ...
    if (
      ...
      if (getT >= Ti) {
        ...
        if (getLamda == 2 * getMu) {
          ...
          document.getElementById("nt").innerHTML = `n(${getT}) = ${
            getK - 1
          } or ${getK - 2}`;
        }
      } else {
        let setNt =
          Math.trunc(getLamda * getT) -
          Math.trunc(getMu * getT - getMu / getLamda);
      ...
      if (getN == 0) {
        ...
        if (getLamda == 2 * getMu) {
          ...
          document.getElementById(
            "wn"
          ).innerHTML = `Wq(${getN}) = ${setWn1} or ${setWn2}`;
        }
      }
    }
  } // lamda <= mu
    ...
    if (
      ...
      if (getM <= 0) {
        ...
        if (getT < Ti) {
          let setNt =
            getM + Math.trunc(getLamda * getT) - Math.trunc(getMu * getT);
          document.getElementById(
            "nt"
          ).innerHTML = `n(${getT}) Where ${getM} initial = ${setNt}`;
        } else {
          document.getElementById(
            "nt"
          ).innerHTML = `n(${getT}) Where ${getM} initial = 0 or 1`;
        }
        ...
        if (getN == 0) {
          ...
          document.getElementById(
            "wn"
          ).innerHTML = `Wq(${getN}) = ${setWq.toFixed(2)}`;
        } else {
          document.getElementById("wn").innerHTML = `Wq(${getN}) = 0`;
        }
      }
...
function calcMM1kBtn() {
  ...
  if (
    ...
    if (row === 1) {
      ...
      let up =
        1 - (getK + 1) * Math.pow(row, getK) + getK * Math.pow(row, getK + 1);
...
function calcMMcBtn() {
  ...
  if (
    ...
    if (row < 1) {
      ...
      let oneDivPo =
        firstPart + (getC * Math.pow(R, getC)) / (factorial(getC) * (getC - R));
      Po = 1 / oneDivPo;
      ...
      let oneDivPo =
        ...
        (1 / factorial(getC)) *
          Math.pow(R, getC) *
          ((getC * getMu) / (getC * getMu - getLamda));
      Po = 1 / oneDivPo;
    }
...
function calcMMckBtn() {
  ...
  if (
    ...
    if (row == 1) {
      ...
      let oneDivPo =
        firstPart + (Math.pow(R, getC) / factorial(getC)) * (getK - getC + 1);
      Po = 1 / oneDivPo;
    } else {
      let firstPart = 0;
      for (let i = 0; i <= getC - 1; i++) {
      ...
      let oneDivPo =
        firstPart +
        (Math.pow(R, getC) / factorial(getC)) *
          ((1 - Math.pow(row, getK - getC + 1)) / (1 - row));
      Po = 1 / oneDivPo;
    }
    console.log(`Po = ${Po}`);
    let Pk =
      (Math.pow(R, getK) / (Math.pow(getC, getK - getC) * factorial(getC))) *
      Po;
    let lamdaDash = getLamda * (1 - Pk);
    let firstPartLq =
      (row * Math.pow(R, getC) * Po) / (factorial(getC) * Math.pow(1 - row, 2));
    let setLq =
      firstPartLq *
      (1 -
        Math.pow(row, getK - getC + 1) -
        (1 - row) * (getK - getC + 1) * Math.pow(row, getK - getC));
    let secondPartL = 0;
    for (let i = 0; i <= getC - 1; i++) {
